...
from appwrite_client_python_sdk.exceptions_base import ApiValueError
from appwrite_client_python_sdk.exceptions import ClientConfigurationError
...
conf = appwrite_client_python_sdk.Configuration(
    ...
    def __init__(self, host=None,
        ...
        """Constructor
        """
        self.host = "https://cloud.appwrite.io/v1" if host is None else host
        """Default Base url
        """
        ...
        self.api_key = {}
        if jwt:
            self.api_key['JWT'] = jwt
        if locale:
            self.api_key['Locale'] = locale
        if project:
            self.api_key['Project'] = project
        else:
            raise ClientConfigurationError('API Key "Project" is required')
        if session:
            self.api_key['Session'] = session
        ...
        """Logging Settings
        """
        self.logger = logging.getLogger("appwrite_client_python_sdk")
    ...
    def auth_settings(self):
        ...
        if 'Locale' in self.api_key:
            auth['Locale'] = {
                'type': 'api_key',
                'in': 'header',
                'key': 'X-Appwrite-Locale',
                'value': self.get_api_key_with_prefix(
                    'Locale',
                ),
            }
        if 'Project' in self.api_key:
            auth['Project'] = {
                'type': 'api_key',
                'in': 'header',
